...
(function (window) {
  ...
  FlyObj.factory = function (type, options) {
    switch (type) {
      case 'Bird':
        return new FlyObj.Bird(options);
      case 'Sky':
      return new FlyObj.Sky(options);
      case 'Land':
        return new FlyObj.Land(options);
      case 'Pipe':
      return new FlyObj.Pipe(options);
    }
  }
  // 暴露到全局环境中
  window.Fly = FlyObj;
